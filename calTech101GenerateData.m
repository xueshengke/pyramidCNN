...
%% data path
imagePath = '../Caltech101/101_ObjectCategories/';
className = {
...
labelsTrain = zeros(classNumber, imageMap, trainNumber * classNumber);
labelsTest  = zeros(classNumber, imageMap, testNumber  * classNumber);
...
for i = 1 : classNumber
    ...
    labelsTrain(:, :, 1 + (i - 1) * trainNumber : i * trainNumber) = ...
        repmat(labels(:, i), [1 imageMap trainNumber]);
    labelsTest(:, :, 1 + (i - 1) * testNumber : i * testNumber) = ...
        repmat(labels(:, i), [1 imageMap testNumber]);
end
...
randNumber = randperm(size(dataTrain, 3));
